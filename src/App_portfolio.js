...
function PortfolioApp() {
  ...
  useEffect(() => {
    ...
    const HERO_SCROLL_MULTIPLIER = 3; // 2x or 3x slowdown for hero contraction
    ...
    const heroTl = gsap.timeline({
      ...
      scrollTrigger: {
        trigger: '.hero',
        start: 'top top',
        end: `+=${400 * HERO_SCROLL_MULTIPLIER}vh`, // Slow down by increasing scroll distance
        pin: true,
        pinSpacing: true,
    ...
    // Parallax drift for lines during the hold phase (40% -> 100%)
    heroTl.fromTo(line1Ref.current, { y: 0 }, { y: -30, ease: 'none', duration: 0.6 }, 0.4);
    heroTl.fromTo(nameRef.current,  { y: 0 }, { y: -60, ease: 'none', duration: 0.6 }, 0.4);
    heroTl.fromTo(line3Ref.current, { y: 0 }, { y: -20, ease: 'none', duration: 0.6 }, 0.4);
    ...
    const setupProgressiveProjectsScrolling = () => {
      ...
      const applyScrollLogic = () => {
        ...
        ScrollTrigger.create({
          ...
          scrub: 0.6,            // Smoother entry with more resistance
          anticipatePin: 1,
          markers: debugMode ? {
            startColor: 'blue',
            endColor: 'blue',
        ...
        ScrollTrigger.create({
          ...
          scrub: 1.2, // Higher scrub value for minimal interference with natural scroll
          anticipatePin: 1,
          markers: debugMode ? {
            startColor: 'blue',
            endColor: 'blue',
        ...
        ScrollTrigger.create({
          ...
          trigger: projectsSection,
          start: 'top top',
          end: '+=800vh', // Longer track for smoother scrolling
          pin: true, // Re-enabled for scroll effects
          scrub: 1.0, // Normal vertical scroll sensitivity
          anticipatePin: 1,
          snap: {
            snapTo: (progress) => {
              ...
              // Magnetic snapping: always snap to nearest snap point when in commit zones
              const snapPoints = [0, 0.5, 1];
              const midpoints = [0.25, 0.75]; // 50-50 points between snap points
              
              // Wider tolerance (0.5) for easier snapping - snaps with less scroll
              const isNearMidpoint = midpoints.some(midpoint => Math.abs(progress - midpoint) < 0.5);
              const isNearSnapPoint = snapPoints.some(snapPoint => Math.abs(progress - snapPoint) < 0.5);
              
              // Always snap to nearest point if we're anywhere near it
              if (isNearMidpoint || isNearSnapPoint) {
                // Find the nearest snap point for magnetic snapping
                return snapPoints.reduce((prev, curr) => 
                  Math.abs(curr - progress) < Math.abs(prev - progress) ? curr : prev
                );
              }
              
              // If not near any zone, still snap to nearest point for responsive snapping
              return snapPoints.reduce((prev, curr) => 
                Math.abs(curr - progress) < Math.abs(prev - progress) ? curr : prev
              );
            },
            duration: { min: 0.3, max: 0.6 }, // Faster snap response
            delay: 0.05, // Minimal delay for quick snapping
            ease: "power2.out", // Quick, responsive snap
            directional: false, // Allow snapping in both directions
            inertia: false, // User controls the scroll, snap assists when stopping
            snapDelay: 0 // No delay for immediate snap response in both directions
          },
          markers: debugMode ? {
            startColor: "green",
            endColor: "green",
          ...
          onUpdate: (self) => {
            ...
            const isScrollingDown = progress > lastScrollProgressRef.current;
            ...
            // Only one project active (full opacity) at a time - even during scrolling
              ...
              if (isAtSnapPoint) {
                ...
                if (index === activeCardIndex) {
                  ...
                  // Set opacity immediately when at snap point
                  ...
                  gsap.to(card, { 
                    ...
                    ease: "power2.out",
                    onComplete: () => {
                      // Re-enforce after opacity animation completes
                ...
                // Set opacity immediately to ensure only one project is active
                if (index === activeCardIndex) {
                  gsap.set(card, { opacity: 1 }); // Set immediately
                  ...
                  gsap.set(card, { opacity: 0.3 }); // Set immediately
                  card.style.pointerEvents = 'none';
                  gsap.to(card, { 
                    opacity: 0.3, 
                    duration: 0.15,
        ...
        ScrollTrigger.create({
          ...
          snap: {
            snapTo: (progress) => {
              ...
              const midpoints = [0.165, 0.495, 0.83]; // 50-50 points between snap points
          ...
          onUpdate: (self) => {
            ...
            // Track scroll direction
            const isScrollingDown = progress > lastMiniScrollProgressRef.current;
    ...
    gsap.set('.contact-options-kicker', { color: '#111111' });
    ...
    const activateDarkMode = () => {
      ...
      gsap.to('body', { backgroundColor: '#000000', duration: 0.05, ease: 'power2.out' });
      gsap.to('.marquee-contact-section', { backgroundColor: '#000000', color: '#ffffff', duration: 0.05, ease: 'power2.out' });
      gsap.to('.marquee-text', { color: '#ffffff', duration: 0.05, ease: 'power2.out' });
      gsap.to('.contact-options-kicker', { color: '#ffffff', duration: 0.05, ease: 'power2.out' });
            // Only target SVG paths in the about section, not contact section
            gsap.to('.about .about-title-line .morphing-icon svg path', { stroke: '#ffffff', duration: 0.05, ease: 'power2.out' });
            gsap.to('.about .about-title-line .morphing-icon.icon-fill svg path', { fill: '#ffffff', duration: 0.05, ease: 'power2.out' });
      
      // Apply white filter to arrow SVGs and contact section PNG images
      gsap.to('.about-title-line .arrow svg', { filter: 'brightness(0) invert(1)', duration: 0.05, ease: 'power2.out' });
      gsap.to('.marquee-contact-section .category-svg img', { filter: 'brightness(0) invert(1)', duration: 0.05, ease: 'power2.out' });
    ...
    const activateLightMode = () => {
      ...
      gsap.to('body', { backgroundColor: '#f5f5f0', duration: 0.05, ease: 'power2.out' });
      gsap.to('.marquee-contact-section', { backgroundColor: '#f5f5f0', color: '#111111', duration: 0.05, ease: 'power2.out' });
      gsap.to('.marquee-text', { color: '#111111', duration: 0.05, ease: 'power2.out' });
      gsap.to('.contact-options-kicker', { color: '#111111', duration: 0.05, ease: 'power2.out' });
            // Only target SVG paths in the about section, not contact section
            gsap.to('.about .about-title-line .morphing-icon svg path', { stroke: '#111111', duration: 0.05, ease: 'power2.out' });
            gsap.to('.about .about-title-line .morphing-icon.icon-fill svg path', { fill: '#111111', duration: 0.05, ease: 'power2.out' });
      
      // Remove filter from arrow SVGs and contact section PNG images
      gsap.to('.about-title-line .arrow svg', { filter: 'none', duration: 0.05, ease: 'power2.out' });
      gsap.to('.marquee-contact-section .category-svg img', { filter: 'none', duration: 0.05, ease: 'power2.out' });
  ...
  useEffect(() => {
    ...
    const animateTrail = () => {
      ...
      if (isHoveringProject && hoveredElement && hoveredElement.isConnected) {
        // Magnetic effect - snap to project center with stronger attraction
    ...
    const setupAboutHoverListeners = () => {
      ...
      aboutRows.forEach((row) => {
        ...
        const mouseenterHandler = () => {
          ...
          isHoveringProject = true;
          // Use only the word SVG container for sizing/centering
          const wordEl = row.querySelector('.category-svg') || row;
          hoveredElement = wordEl;
          if (trail && trail.isConnected) {
            trail.classList.add('about-rect');
            // Match trail size to the word bounds with slight padding
            const rect = wordEl.getBoundingClientRect();
            const paddingPx = 3; // 2–3px padding from text to border
            trail.style.width = rect.width + paddingPx * 2 + 'px';
            trail.style.height = rect.height + paddingPx * 2 + 'px';
          }
  ...
  const getSectionData = (section) => {
    const data = {
      psychology: {
        ...
        body: 'Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum'
      },
      design: {
        icon: <DesignIcon />,
        title: 'Lorem Ipsum Lorem Ipsum',
  ...
  return (
    <>
      ...
      <section className="hero" style={debugMode ? { border: '2px solid red' } : {}}>
        <div className="video-container" ref={videoContainerRef} style={debugMode ? { border: '2px solid blue' } : {}}>
          <video ref={videoRef} src="/video.mp4" autoPlay muted playsInline style={debugMode ? { border: '2px solid green' } : {}} />
        </div>
              ...
              >
                ...
                >
                  ...
                  <div className="project-info" style={debugMode ? { border: '2px solid indianred' } : {}}>
                    <h3 className="project-title" style={debugMode ? { border: '2px solid firebrick' } : {}}>Cognixa: Building Alone, Building User-First</h3>
                    <p className="project-subtitle" style={debugMode ? { border: '2px solid crimson' } : {}}>From Psychology to Product, A UX first approach into Systems Thinking and Data-Driven Decision Making</p>
                  </div>
                </Link>
                {hoveredProject === 'cognixa' && (
                  <div className="project-tooltip">Click to learn how I did this</div>
                )}
              </div>
              ...
              >
                ...
                >
                  ...
                  <div className="project-info" style={debugMode ? { border: '2px solid forestgreen' } : {}}>
                    <h3 className="project-title" style={debugMode ? { border: '2px solid seagreen' } : {}}>Settlin : Designing with a product mindset</h3>
                    <p className="project-subtitle" style={debugMode ? { border: '2px solid mediumseagreen' } : {}}>My learnings that helped me think in systems and wholistic ecosystem rather than free standing domains</p>
                  </div>
                </Link>
                {hoveredProject === 'settlin' && (
                  <div className="project-tooltip">Click to learn how I did this</div>
                )}
              </div>
              ...
              >
                ...
                >
                  ...
                  <div className="project-info" style={debugMode ? { border: '2px solid steelblue' } : {}}>
                    <h3 className="project-title" style={debugMode ? { border: '2px solid royalblue' } : {}}>Settlin : Designing with a product mindset</h3>
                    <p className="project-subtitle" style={debugMode ? { border: '2px solid dodgerblue' } : {}}>My learnings that helped me think in systems and wholistic ecosystem rather than free standing domains</p>
                  </div>
                </Link>
        ...
        <div className="contact-options-wrapper">
          <div className="contact-options-header">
            <div className="contact-options-kicker">
              <div style={{ color: '#FFFFFF', opacity: 1 }}>LET'S CREATE SOMETHING TOGETHER.</div>
          ...
          <div className="contact-options-title">
            <div className="about-title-line" data-section="email">
              ...
              <span className="arrow">
                <span className="icon-base">
                  <MorphingIcon className="morphing-icon" sectionName="email" />
                </span>
                <span className="fill-clip">
                  <MorphingIcon className="morphing-icon icon-fill" sectionName="email" filled />
                </span>
              </span>
            </div>
            ...
            <div className="about-title-line" data-section="schedule">
              ...
              <span className="arrow">
                <span className="icon-base">
                  <MorphingIcon className="morphing-icon" sectionName="schedule" />
                </span>
                <span className="fill-clip">
                  <MorphingIcon className="morphing-icon icon-fill" sectionName="schedule" filled />
                </span>
              </span>
            </div>
            ...
            <div className="about-title-line" data-section="message">
              ...
              <span className="arrow">
                <span className="icon-base">
                  <MorphingIcon className="morphing-icon" sectionName="message" />
                </span>
                <span className="fill-clip">
                  <MorphingIcon className="morphing-icon icon-fill" sectionName="message" filled />
                </span>
              </span>
            </div>
          </div>
